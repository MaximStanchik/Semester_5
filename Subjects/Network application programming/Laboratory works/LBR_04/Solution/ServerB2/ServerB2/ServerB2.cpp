...
bool GetRequestFromClient(char* name, short port, struct sockaddr* from, int* flen) {
    ...
    while (true) {
        ...
        if (strncmp(buf, "SERVER:", 7) == 0) {
            cout << "Received server message: " << buf << endl;
            continue; 
        }

        cout << "Received message from: " << inet_ntoa(((SOCKADDR_IN*)from)->sin_addr) << " - " << buf << endl;
    }
}
...
void GetServer(char* call, short port, struct sockaddr* from, int* flen)
{
    ...
    try
    {
        ...
        all.sin_addr.s_addr = inet_addr("172.20.10.15"); // широковещательный адрес
...
// показать в паре что я запускаю один сервер, а потом второй. счетчик увеличивается. подключаем клиента широковещательно. позывной одинаковый. у сервера в консоли выводить адрес порт и сообщение от клиента
