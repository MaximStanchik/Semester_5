...
CREATE TABLE PRODUCTS (
    ...
    PRODUCT_ID CHAR(5) NOT NULL, -- идентификатор продукта
    DESCRIPTION VARCHAR(20) NOT NULL, -- описание продукта
    PRICE DECIMAL(9,2) NOT NULL, -- цена продукта
    QTY_ON_HAND INTEGER NOT NULL, -- количество товара на складе
    PRIMARY KEY (MFR_ID, PRODUCT_ID)
);

-- инфомрация об офисах
CREATE TABLE OFFICES (
     OFFICE INTEGER NOT NULL, -- номер офиса
     CITY VARCHAR(15) NOT NULL, -- город, где расположен офис
     REGION VARCHAR(10) NOT NULL, -- регион
     MGR INTEGER, -- идентификатор менеджера офиса
     TARGET DECIMAL(9,2), -- целевая сумма продаж
     SALES DECIMAL(9,2) NOT NULL, -- общая сумма продаж
     PRIMARY KEY (OFFICE) 
);

-- информация о сотрудниках
CREATE TABLE SALESREPS ( 
	EMPL_NUM INTEGER NOT NULL, CHECK (EMPL_NUM BETWEEN 101 AND 199), -- номер сотрудника
    NAME VARCHAR(15) NOT NULL, -- имя сотрудника
    AGE INTEGER, -- возраст сотрудника
    REP_OFFICE INTEGER, -- номер офиса, в котором работает сотрудник
    TITLE VARCHAR(10), -- должность сотрудника
    HIRE_DATE DATE NOT NULL, -- дата найма сотрудника
    MANAGER INTEGER, -- идентификатор менеджера
    QUOTA DECIMAL(9,2), -- квота по продажам
    SALES DECIMAL(9,2) NOT NULL, -- общая сумма продаж
...
CREATE TABLE ORDERS (
	ORDER_NUM INTEGER NOT NULL, CHECK (ORDER_NUM > 100000), -- номер заказа
  	ORDER_DATE DATE NOT NULL, -- дата заказа
    CUST INTEGER NOT NULL, -- номер клиента
    REP INTEGER, -- идентификатор менеджера по продаже
    MFR CHAR(3) NOT NULL, -- идентификатор производителя
